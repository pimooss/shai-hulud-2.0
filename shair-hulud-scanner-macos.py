#!/usr/bin/env python3
"""
Shai-Hulud Supply Chain Attack Scanner for macOS
...
IOC_PATHS = [
    "~/.bun/bin/bun",  # Malicious Bun installation
    "cloud.json",
    "contents.json",
    "environment.json",
    "truffleSecrets.json",
...
def print_banner():
    ...
    print("    macOS NPM Package Vulnerability Detector")
    print("=" * 70)
    print(f"{Color.END}\n")
...
def find_package_json_files(start_path: str = os.getcwd()) -> List[Path]:
    ...
    # Common locations for npm packages on macOS
    search_paths = [
        start_path,
...
def scan_for_malicious_files() -> List[Tuple[Path, str]]:
    ...
    # Search in common npm installation directories
    search_dirs = [
        os.path.expanduser("~/.npm"),
...
def check_ioc_files() -> List[Path]:
    ...
    for ioc_path in IOC_PATHS:
        ...
        if Path(ioc_path).exists():
            found_iocs.append(Path(ioc_path))
...
def check_github_workflows() -> List[Path]:
    ...
    if workflows_dir.exists():
        ...
        for workflow_file in workflows_dir.glob("*.yml"):
            if workflow_file.name == "discussion.yml" or workflow_file.name.startswith("formatter_"):
                suspicious_workflows.append(workflow_file)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Shai-Hulud Supply Chain Attack Scanner')
    ...
    results = {
        ...
        "scan_date": subprocess.run(['date', '+%Y-%m-%d %H:%M:%S'], capture_output=True, text=True).stdout.strip(),
        "vulnerable_packages": {},
    ...
    for pkg_name, version in global_packages.items():
        if pkg_name in COMPROMISED_PACKAGES:
            ...
            if normalized_version in vulnerable_versions:
                global_vulns.append((pkg_name, version, True))
                ...
                global_vulns.append((pkg_name, version, False))
                results["global_vulnerabilities"].append({
                    "package": pkg_name,
                    "version": version,
                    "exact_match": False
    ...
    print(f"\n{Color.BOLD}[6/7] Checking for malicious GitHub runners...{Color.END}")
    ...
    print(f"\n{Color.BOLD}[7/7] Checking for attacker-created repositories...{Color.END}")
